...
if (storedTeddies == null || storedTeddies.length === 0) {
    ...
    function generateFirstNameElt(selectorClass, label, type) {
        const divFirstName = document.createElement('div');
        form.appendChild(divFirstName);
        divFirstName.className = selectorClass;

        const labelFirstName = document.createElement('label');
        divFirstName.appendChild(labelFirstName);
        labelFirstName.setAttribute('for', label);
        labelFirstName.textContent = 'Votre prénom : ';

        const firstName = document.createElement(type);
